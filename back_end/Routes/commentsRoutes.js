...
router.post('', (req, res) => {
	...
	db.insert({content, date, posted_by, event_id }).into('comments')
		.then(res1 => {
...
router.delete('', (req, res) => {
	...
	.then(() => {
		...
		.then(resp1 => {
			...
			.catch(err3 => {//catch for error on decrementing comment count on event
				console.log(err3)
				res.status(500).json(err3)
			})
		})
		.catch(err2 => {//catch for error on looking up event
			console.log(err2)
			res.status(500).json(err2)
		})
	})
	.catch(err1 => { //catch for error on deleting comment
		console.log(err1)
		res.status(500).json(err1)
	})
})
