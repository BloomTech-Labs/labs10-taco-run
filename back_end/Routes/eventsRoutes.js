...
//-------------------------------------------
router.post('', (req, res) => {
	const {name, date, author, user_id, posters_email, posters_pic, invite_only } = req.body;
	...
	.then(check => {
		...
		if (check.length === 0){
			db.insert({name, date, author, user_id, posters_email, posters_pic, invite_only }).into('events')
			.then(() => {
				db('events')
